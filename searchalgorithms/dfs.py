...
class dfs(SearchAlgorithmBase):
    ...
    def reset(self, grid, start, goal): 
        # Initialize the DFS algorithm with the given grid, start, and goal positions
    ...
    def step(self):
        # Perform one step of the DFS algorithm
        if self._done:
            return
        ...
        node, h, g, parent = self._frontier.pop()  # stack behavior
        ...
        # For DFS, push neighbors onto the stack. Reversed order makes traversal deterministic.
        neighbors = list(self._neighbors(node))
        neighbors.reverse()
        for nbr in neighbors:
            ...
            self._frontier.append((nbr, 0, g + 1, node))
